...
import { submitVideo, updateVideo } from '../../services/apis/videoService';
...
interface VideoFormProps {
  ...
  editMode?: boolean; // Whether we're editing an existing video
}

const VideoForm: React.FC<VideoFormProps> = ({ onClose, onCancel, form, setForm, errors, setErrors, editMode = false }) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      console.error(editMode ? 'Update' : 'Submit', 'error:', error);
      setStatus({ success: false, message: error.message || (editMode ? "Failed to update video." : "Failed to submit video.") });
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-8 bg-gray-900 p-8 rounded-lg border border-yellow-700 shadow-inner" noValidate>
          ...
          <div>
            ...
            <input id="title" name="title" type="text" value={form.title} onChange={handleChange} className={`w-full border rounded-md px-4 py-3 focus:outline-none focus:ring-2 bg-black text-yellow-300 placeholder-yellow-600 ${errors.title ? "border-red-600 focus:ring-red-600" : "border-yellow-700 focus:ring-yellow-600"}`} required />
          ...
          <div className="flex justify-end gap-4 mt-8 items-center">
            ...
            <button type="submit" className="bg-yellow-400 hover:bg-yellow-500 text-black px-6 py-2 rounded font-fell font-bold" disabled={loading}>
              {loading ? "Saving..." : (editMode ? "Update Video" : "Save Video")}
            </button>
